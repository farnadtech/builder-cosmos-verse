...
export default function CreateProject() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        setErrors({ ...errors, attachment: "حجم فایل نباید بیش از ۱۰ مگابایت باشد" });
        return;
      }
  ...
  const validateForm = () => {
    ...
    if (Math.abs(totalMilestonesAmount - budget) > 0.01) {
      newErrors.milestones_total = "مجموع مبالغ مراحل باید برابر بودجه کل پروژه باشد";
      hasMilestoneError = true;
    }
                ...
                <div className="space-y-2">
                  ...
                  {formData.budget && (
                    <p className="text-sm text-gray-600">
                      {parseFloat(formData.budget).toLocaleString('fa-IR')} ریال
                    </p>
                  )}
                  {errors.budget && (
                    <p className="text-sm text-red-500 flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={errors[`milestone_title_${index}`] ? "border-red-500" : ""}
                      />
                      {errors[`milestone_title_${index}`] && (
                        <p className="text-sm text-red-500">{errors[`milestone_title_${index}`]}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={errors[`milestone_amount_${index}`] ? "border-red-500" : ""}
                      />
                      {errors[`milestone_amount_${index}`] && (
                        <p className="text-sm text-red-500">{errors[`milestone_amount_${index}`]}</p>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {formData.budget && (
                  <div className="flex justify-between items-center text-sm text-gray-600 mt-1">
                    ...
                    <span>{parseFloat(formData.budget).toLocaleString('fa-IR')} ریال</span>
                  </div>
                )}
