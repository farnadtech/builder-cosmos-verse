...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    if (user) {
      ...
      window.authenticatedFetch = async (
        url: string,
        options: RequestInit = {},
      ) => {
  ...
  const register = async (
    ...
    try {
      ...
      if (!data.ok) {
        const errorMessage =
          data.data?.message || data.data?.messageFA || "خطا در ثبت نام";
      ...
      if (data.data.success) {
        ...
        return {
          success: true,
          message: data.data.message || "ثبت نام با موفقیت انجام شد",
        };
      } else {
        return {
          success: false,
          message: data.data.message || "خطا در ثبت نام",
        };
      }
    } catch (error) {
