    ...
    if (userRole === 'employer') {
      ...
      if (arbitrationStats.rows.length > 0) {
        stats = {
          ...
          activeProjects: parseInt(arbitrationStats.rows[0].active_cases) || 0,
          completedProjects: parseInt(arbitrationStats.rows[0].resolved_cases) || 0
        };
      }
      ...
      if (adminStats.rows.length > 0) {
        stats = {
          ...
          totalTransactions: parseInt(adminStats.rows[0].total_transactions) || 0,
          platformRevenue: parseFloat(adminStats.rows[0].platform_revenue) || 0
        };
      }
    }
...
router.get('/recent-projects', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const userRole = req.user?.role;
    ...
    if (userRole === 'employer') {
      ...
      projects = result.rows.map(row => ({
        ...row,
        contractor: row.contractor_first_name && row.contractor_last_name 
      ...
      projects = result.rows.map(row => ({
        ...row,
        employer: row.employer_first_name && row.employer_last_name 
